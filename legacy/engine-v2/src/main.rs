...
//!
//! ```bash
//! rave -i input.mp4 -o output.mp4 -m model.onnx
//! rave -i input.265 -o output.265 -m model.onnx --codec hevc --width 1920 --height 1080
//! ```
...
use rave_engine::core::backend::UpscaleBackend;
use rave_engine::codecs::nvdec::NvDecoder;
use rave_engine::codecs::nvenc::{NvEncConfig, NvEncoder};
use rave_engine::codecs::sys::cudaVideoCodec;
use rave_engine::core::context::GpuContext;
...
use rave_engine::io::file_sink::FileBitstreamSink;
use rave_engine::io::file_source::FileBitstreamSource;
use rave_engine::io::ffmpeg_demuxer::FfmpegDemuxer;
use rave_engine::io::ffmpeg_muxer::FfmpegMuxer;
use rave_engine::io::probe_container;

// ─── CLI argument definition ─────────────────────────────────────────────────

/// RAVE — GPU-native video super-resolution engine.
///
...
#[derive(Parser, Debug)]
#[command(name = "rave", version, about)]
struct Cli {
...
async fn run_pipeline(cli: Cli) -> Result<()> {
    ...
    let precision = match cli.precision.to_lowercase().as_str() {
        ...
        other => {
            return Err(rave_engine::error::EngineError::Pipeline(format!(
    ...
    let source: Box<dyn rave_engine::codecs::nvdec::BitstreamSource> = if input_is_container {
    ...
    let sink: Box<dyn rave_engine::codecs::nvenc::BitstreamSink> = if output_is_container {
...
fn parse_codec(s: &str) -> Result<cudaVideoCodec> {
    match s.to_lowercase().as_str() {
        ...
        other => Err(rave_engine::error::EngineError::Pipeline(format!(
