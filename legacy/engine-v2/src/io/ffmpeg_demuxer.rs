...
use crate::io::ffmpeg_sys::{
    check_ffmpeg, to_cstring,
    // BSF FFI (missing from ffmpeg-sys-next v8)
    AVBSFContext, av_bsf_alloc, av_bsf_free, av_bsf_get_by_name,
...
impl FfmpegDemuxer {
    ...
    fn rescale_pts(&self, pts: i64) -> i64 {
        ...
        let us_tb = AVRational { num: 1, den: 1_000_000 };
            ...
            if ret < 0 {
                ...
                if ret == AVERROR_EOF {
                    ...
                    if !self.bsf_ctx.is_null() {
                        ...
                        let ret2 = unsafe {
                            av_bsf_receive_packet(self.bsf_ctx, self.pkt_filtered)
                        };
                        if ret2 == 0 {
                            let pkt = unsafe { &*self.pkt_filtered };
                            let data = unsafe {
